...
export {
	...
	global send_subscribe: function(topic: string, query: ZeekAgent::Query);

	## Send a message to an zeek-agent or group of hosts to unsubscribe from a query
	##
	## topic: The topic of the host or group to address
	## query: The query to unsubscribe from
	global send_unsubscribe: function(topic: string, query: ZeekAgent::Query);

	## Send a message to an zeek-agent or group of hosts to execute a query
	##
	## topic: The topic of the host or group to address
	## query: The query to execute
	global send_execute: function(topic: string, q: ZeekAgent::Query);
...
function send_subscribe(topic: string, query: ZeekAgent::Query)
	{
	local ev_name = split_string(fmt("%s", query$ev), /\n/)[0];
	local host_topic = topic;
	
	ZeekAgent::log("debug", topic, fmt("%s event %s() for query '%s'", "Subscribing to", ev_name, query$query), LOG_SEND);
	
	local update_type = "BOTH";
	if ( query$utype == ZeekAgent::ADD )
		update_type = "ADDED";
	
	if ( query$utype == ZeekAgent::REMOVE )
		update_type = "REMOVED";
	...
	local inter = double_to_count(interval_to_double(query$inter));
...
function send_unsubscribe(topic: string, query: ZeekAgent::Query)
	{
	local ev_name = split_string(fmt("%s", query$ev), /\n/)[0];
	local host_topic = topic;
	
	ZeekAgent::log("debug", topic, fmt("%s event %s() for query '%s'", "Unsubscribing from", ev_name, query$query), LOG_SEND);
...
function send_execute(topic: string, q: ZeekAgent::Query)
	{
	local ev_name = split_string(fmt("%s", q$ev), /\n/)[0];
	local host_topic = topic;
	
	ZeekAgent::log("debug", topic, fmt("%s event %s() for query '%s'", "Executing", ev_name, q$query), LOG_SEND);
...
function send_join(host_topic: string, group: string)
	{
	ZeekAgent::log("info", host_topic, fmt("%s group '%s'", "Joining", group), LOG_SEND);
...
function send_leave(host_topic: string, group: string)
	{
	ZeekAgent::log("info", host_topic, fmt("%s group '%s'", "Leaving", group), LOG_SEND);
...
event zeek_init()
	{
	Log::create_stream(LOG_SEND, [$columns=ZeekAgent::Info, $path="zeek-agent-hosts"]);
	}
